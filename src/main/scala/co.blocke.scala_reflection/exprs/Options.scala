  ...
  def makeExpr[T](opt: OptionRType[T])(using q:Quotes)(using Type[T]): Expr[RType[T]] = 
    ...
    opt match {
        case scalaOpt: ScalaOptionRType[_] =>
            ...
            val optTypeExpr = ExprMaster.makeExpr(scalaOpt._optionParamType)(using q)(using tt.asInstanceOf[Type[scalaOpt._optionParamType.T]]).asInstanceOf[Expr[RType[scalaOpt._optionParamType.T]]]
