  ...
  private def _show(
    ...
    rt match {
      ...
      case t: MapRType[?] =>
        ...
        buf.append(tabs(tabLevel + 1))
        buf.append("key: ")
        val (_, lastWasMultiLine_1, classesSeen_1) = _show(t.elementType, buf, tabLevel + 1, seenBefore)
        if !lastWasMultiLine_1 then buf.append("\n")
        buf.append(tabs(tabLevel + 1))
        buf.append("value: ")
        val (_, lastWasMultiLine_2, classesSeen_2) = _show(t.elementType2, buf, tabLevel + 1, classesSeen_1)
