...
class ScalaTasty extends munit.FunSuite:
  ...
  test("Classes defined inside objects should work (class input)") {
    val result = RType.of(classOf[BigObject.LittleThing])
    assertEquals(result.infoClass.getName(), "co.blocke.scala_reflection.BigObject$LittleThing")
    assertEquals(result.show(),
      """ScalaCaseClassInfo(co.blocke.scala_reflection.BigObject$LittleThing):
        |   fields:
        |      (0) a: scala.Int
        |""".stripMargin)
  }