...
object JavaStack:
  ...
  def makeExpr[T](jstack: JavaStackRType[T])(using q: Quotes)(using Type[T]): Expr[RType[T]] =
    ...
    val tt = jstack.elementType.toType(quotes)
    val jstackTypeExpr = stripType(
      ExprMaster
        .makeExpr(jstack.elementType)(using q)(using tt.asInstanceOf[Type[jstack.elementType.T]])
        .asInstanceOf[Expr[RType[jstack.elementType.T]]]
    )
