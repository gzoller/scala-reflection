...
object ReflectOnField:
  ...
  def nonCaseScalaField[Q](quotes: Quotes)( symbol: quotes.reflect.Symbol, classRepr: quotes.reflect.TypeRepr ): List[NonConstructorFieldInfo] = 
    import quotes.reflect.* 
    ...
    symbol.methodMembers.filter(_.name.endsWith("_=")).map{ setter =>
    ...
    }.zipWithIndex.map{ case ((getter, setter),i) =>   
  ...
  def javaFields[Q](quotes: Quotes)( symbol: quotes.reflect.Symbol ): List[NonConstructorFieldInfo] = 
    ...
    getterMethods.zip(setterMethods).collect {
