...
* Make Scala 3 reflection a little more approachable by exposing higher-level abstractions for reflected things (vs diving through Scala 3 classes that mirror compiler internals)
...
* The entire serialized RType tree (incl. any nested RTypes) must not exceed 64K bytes. This is so that it will fit into a Java Annotation.  (If this becomes a frequent point of limitation, there may be a way to extend this limitation, but we have no desire to prematurely over-engineer.  Until we learn otherwise, 64K seems a reasonable amount.)
