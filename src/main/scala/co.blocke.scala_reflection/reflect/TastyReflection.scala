...
object TastyReflection: // extends NonCaseClassReflection:
  ...
  def reflectOnClass(quotes: Quotes)(typeRef: quotes.reflect.TypeRef, typedName: TypedName, resolveTypeSyms: Boolean, appliedTob: List[quotes.reflect.TypeRepr] =  Nil): RType[_] = 
    import quotes.reflect.*
      ...
      // === Scala Classes ===
        ...
        if symbol.flags.is(quotes.reflect.Flags.Case) then
          ...
          val caseFields = constructorParams.zipWithIndex.map{ (definition, idx) =>
            ...
            reflectOnField(quotes)(fieldRType, valDef, idx, dad, fieldDefaultMethods).resolveTypeParams( paramMap )
          }
