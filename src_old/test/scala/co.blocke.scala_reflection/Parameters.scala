...
class Parameters extends munit.FunSuite:
  ...
  test("0-level param substitution") {
    ...
    assertEquals( result.show(), """ScalaCaseClassInfo(co.blocke.scala_reflection.DuoTypes):
    |   fields:
    |      (0)[Q] a: scala.Int
    |      (1)[U] b: scala.Float
    |""".stripMargin)
  ...
  test("1st level param substitution") {
    ...
    assertEquals( result.show(), """ScalaCaseClassInfo(co.blocke.scala_reflection.DuoHolder):
    |   fields:
