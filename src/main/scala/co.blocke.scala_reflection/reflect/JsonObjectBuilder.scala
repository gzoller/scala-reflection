...
object JsonObjectBuilder:
    ...
    private def matchOneType(quotes: Quotes)(sb: StringBuilder, repr: quotes.reflect.TypeRepr, item: Any): Unit =
        ...
        repr.typeSymbol.name match {
            case "String" 
                | "TypedName" 
                | "TypeSymbol" => sb.append(s"\"${item.toString}\"")
            ...
            case "List" =>
                ...
                if item.asInstanceOf[List[_]].isEmpty then
                    sb.append(']')
                else
                    sb.setCharAt(sb.length() - 1, ']')
            ...
            case "Map" =>
                ...
                if item.asInstanceOf[Map[String,_]].isEmpty then
                    sb.append('}')
                else
                    sb.setCharAt(sb.length() - 1, '}')

            case "Option" =>
                if item.asInstanceOf[Option[_]].isEmpty then
                    sb.append("null")
                else
                    repr match {
                        case AppliedType(t, tob) =>
            ...
            case "FieldInfo" | "NonConstructorFieldInfo" =>
                item.asInstanceOf[FieldInfo].asJson(sb)(using quotes)
            
            case _ => // Careful!  This is a bold assumption that everything else is RType[_]!
                item.asInstanceOf[RType[_]].asJson(sb)(using quotes)
        }